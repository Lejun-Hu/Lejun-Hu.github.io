...
**NVLink-C2C** 则是英伟达在最新 Vera Rubin 平台中引入的自研 CPU-GPU 直连技术。在上一代 Grace Hopper（GH200）中，Grace CPU 与 Hopper GPU 之间同样通过 NVLink-C2C 连接，但 Vera Rubin 将这一通道的带宽推向了一个新高度——双向 **1.8 TB/s**，延迟在纳秒级。它是 NVLink 协议在 Chiplet 片间互联层面的延伸，让 Vera CPU 能像访问本地内存一样直接读写 Rubin GPU 的 HBM。NVLink-C2C 是英伟达的私有技术，仅用于自家芯片之间的互联。
...
国产 GPU 的 scale-up 互联起步较晚，但发展速度惊人。受出口管制和供应链安全需求的双重驱动，华为昇腾、寒武纪、壁仞、摩尔线程、沐曦等厂商在过去几年密集推出了各自的互联协议，并在 2025—2026 年间实现了从实验室到大规模集群部署的跨越。与此同时，以中国移动主导的 OISA 开放标准正在尝试构建国产 GPU 间的统一互联生态，目标类似 UALink 在中国的落地。以下是各家方案的具体分析。
...
华为于 2025 年发布的 **CloudMatrix 384 超节点**是这一策略的集中体现：将 384 颗昇腾 910C NPU 和 192 颗鲲鹏 CPU 封装进单一逻辑计算域，通过下一代 **UB（Universal Bus，统一总线）** 实现 TB/s 级的高带宽、低延迟互联。UB 架构融合了此前 HCCS 的缓存一致性能力和华为自研的交换芯片技术，目标是让超节点内的数百颗昇腾芯片像一个整体一样工作——这与 NVLink + NVSwitch 将 72 颗 GPU 组成 NVL72 机柜的思路异曲同工，但华为在规模上直接瞄准了更大的数量级。

软件栈方面，华为的 **CANN（Compute Architecture for Neural Networks）** 全栈自研，HCCL 通信库对标 NCCL。目前昇腾 910C 已大规模量产，CloudMatrix 384 已在多家运营商和互联网企业的数据中心落地。华为的路线图显示将在 2026—2027 年推出 910D/910E 等后续芯片，UB 的带宽和规模将持续提升。

**7. 寒武纪：MLU-Link 的全自研路线**

寒武纪是国内最早一批进入 AI 芯片领域的公司，从终端 IP 起家，逐步扩展到云端训练/推理芯片。其互联协议 **MLU-Link** 完全自研，不依赖 PCIe Switch 或第三方交换芯片，目前已在思元 370 和思元 590 两代产品上量产。

MLU-Link 的具体参数为 4 端口、16 lane、单 lane 50 Gbps，双向总带宽 **200 GB/s**。这一数字与 NVLink 相比差距较大，但寒武纪的定位更侧重于**推理场景和中小规模训练**，而非万卡级大模型预训练。思元 590 采用 7nm 工艺和 MLUarch05 架构，INT8 算力 512 TOPS，FP16 算力约 256 TFLOPS，集群 FP16 算力可达 2.048 PFLOPS。

软件栈为 **NeuWare** 全栈平台，包含 AI 加速算子库 CNNL 和集合通信库 **CNCL**。寒武纪最新一代 MLU-Link 4.0 已在规划中，思元 690 将采用 Chiplet 封装，进一步提升互联带宽和扩展规模。寒武纪的客户主要集中在阿里、字节等互联网大厂的推理业务，训练侧的大规模集群部署尚处于验证阶段。

**8. 壁仞科技：Blink 协议与光交换超节点的差异化路线**

壁仞科技是国产 GPU 厂商中少数同时布局电互联和光互联的企业。其自研的 **Blink** 电互联协议每通道 64 GB/s，支持 4 到 8 通道配置，已搭载于 BR100 和 BR104 两代芯片并实现量产。
...
光互连的核心优势在于突破电信号在铜缆上的距离和密度限制，这对于超大规模组网意义重大——Google 的 TPU OCS 和英伟达 Rubin 平台都在向光互连方向演进。壁仞的策略是将光交换超节点作为其差异化竞争力，在国产算力集群的组网层面占据先发优势。
...
2025 年 12 月，摩尔线程在首届 MUSA 开发者大会上发布了全新 GPU 架构 **"花港"** 及首款基于该架构的云端 AI 加速芯片 **"华山"**。华山芯片支持超十万卡级 AI 工厂部署，其 Scale-up 系统兼容 **MTLink 4.0**，整卡互联带宽达到 **1,314 GB/s**，支持最多 **1,024 卡**互联。这一带宽指标在国产方案中处于领先水平，已接近 NVLink 5 的 1,800 GB/s。
...
沐曦股份成立于 2020 年，创始团队拥有丰富的 GPU 研发经验，产品线覆盖云端训练（曦云 C 系列）、推理（曦思 N 系列）和图形渲染（曦彩 G 系列）。其互联方案 **MetaXLink** 已迭代到第二代，在旗舰产品曦云 C600 上提供约 **900 GB/s** 的互联带宽，配备 7 个接口——这一规格与 H200 的 NVLink 4 基本处于同一量级。
...
如果说 UALink 是海外的"反 NVLink 联盟"，那么 **OISA（Omni-directional Intelligent Sensing Architecture，全向智感互联架构）** 就是中国移动主导的国产对标方案。

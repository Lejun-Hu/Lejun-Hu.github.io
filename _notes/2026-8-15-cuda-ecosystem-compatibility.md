...
两种架构各有利弊，昇腾最终选择走向分离，本质是在"延迟"与"吞吐/利用率"之间做了一次权衡。

**耦合架构的优势在于低延迟和简单**：Cube 与 Vector 同核，两者之间的数据交互在核内即可完成，无需经过核外存储，延迟低；同时架构相对简单，核内数据共享便捷，控制逻辑集中。但它的缺陷也很明显——矩阵计算和向量计算相互耦合，**无法独立调度**。当 Cube 和 Vector 的负载不均衡时（这在真实算子中非常常见，例如 GEMM 的计算量远大于紧随其后的激活函数），就会出现"一方忙、一方闲"的相互等待，计算资源利用率受限。此外，耦合架构也难以针对不同类型的计算独立优化频率和指令流水。

**分离架构的核心优势是解耦带来的高利用率与可扩展性**：AIC 与 AIV 各自拥有独立的 Scalar 单元和独立代码段，系统软件可以对两者**分别调度、并行执行**，互不阻塞，从而在矩阵计算和向量计算之间实现流水线并行，显著提升整体吞吐。同时，分离架构可以针对不同计算类型独立优化，并新增了 BT Buffer 和 FP Buffer，将 Bias 加载、量化参数、ReLU 参数等操作在硬件上固化，减少重复搬运。它的代价则是：AIV 与 AIC 之间必须通过 Global Memory 传递数据，延迟高于核内共享；存储层次增多（AIC 有 7 个内存单元），编程模型更复杂，需要系统软件统一调度两个核的协作。

因此，从耦合走向分离，是昇腾面向**高性能训练场景**做出的演进——训练负载对吞吐量和资源利用率的要求远高于推理，而分离架构"独立调度 + 流水线并行"的特性恰好契合这一需求。从产品代次上看，早期的 Atlas 训练/推理系列采用耦合架构，而新一代的 Atlas A2 训练系列产品则采用了分离架构，印证了分离架构是昇腾硬件演进的方向。
...
- **Scalar Unit（标量计算单元）**：类似一个小型 CPU，负责循环控制、分支判断、地址计算，以及向 Cube/Vector 发射指令。值得注意的是，这种"标量单元负责控制、矩阵/向量单元负责计算"的三元结构，并非昇腾独有——Google 的 TPU 更早就采用了类似的组织方式（TPU 自 2015 年问世，其 Tensor Core 内部同样包含一个标量处理单元、一个矩阵乘法单元 MXU 和一个向量单元 VPU，由标量单元负责指令调度）。由于 TPU 的问世时间早于昇腾（昇腾达芬奇架构于 2018 年才首次商用），华为在设计达芬奇架构时很可能借鉴了 TPU 的这一实现思路。这也反映出 DSA 架构在"用轻量控制单元统一调度重计算单元"这一点上存在相当程度的设计共识。
...
这里需要澄清"离线编译"的确切含义，因为它容易和 CUDA 的编译时机混淆。CUDA 的普通用法（`<<<>>>`）也并非"运行时编译"——kernel 同样是在**构建期**由 nvcc 编译好、以 CUBIN/PTX 形式嵌入可执行文件，运行时只是"加载 + 触发"（真正的运行时编译是 §1.7 讲的 NVRTC 那条特殊路径）。所以两者的区别**不在于"编译发生的时间"**，而在于两个更本质的层面：

- **编译产物的粒度不同**：CUDA 编译出的产物是"单个 kernel 的可执行代码"（CUBIN/PTX），kernel 之间是松散、动态的，host 代码可以在任意时刻、任意顺序地下发任意 kernel。而昇腾 ATC 编译出的 `.om` 是一个**预先固化的完整执行计划**——算子的执行顺序、数据流、内存布局都在编译期确定，运行时只能按这个固化计划整体执行，无法在运行中动态插入新算子或改变执行顺序。

- **执行模型的自由度不同**：CUDA 的 kernel 启动是"即时的、可任意组合的"；昇腾的 `.om` 执行是"整体的、固化的"。这背后的根源正是 §3.2 所说的达芬奇 DSA 架构特性——硬件没有动态调度器，必须由编译器在离线阶段把算子的拆分、映射、数据流全部规划好，硬件才能高效执行。因此昇腾的"离线编译"不是软件工程上的选择，而是**硬件架构决定的必然**。

简言之：CUDA 是"编译单个 kernel，运行时灵活调度"；昇腾是"编译整张计算图，运行时按计划执行"。这一差异是理解昇腾编程体验（无法即时测试单个 kernel、必须走完整编译闭环）的关键。

顺着这个思路，一个自然的疑问是：既然 DSA 架构不支持 host 侧动态下发 kernel，而是需要一次性编译好 `.om` 格式的数据流，那么 host 侧是否只需调用一次 `aclmdlExecute` 就能跑完整个模型？答案是"**取决于编译粒度与应用场景**"。

- **整模型编译 + 单次推理**：如果通过 ATC 把整个模型（例如从 ONNX 转换）编译成一个 `.om`，那么一次推理中 host 侧确实只需 `aclmdlLoadFromFile` 加载一次、`aclmdlExecute` 下发一次，然后等待结果即可。整张计算图在 NPU 上自主完成，host 与 device 之间几乎没有来回交互——这正是 GE 引擎的"图下沉"能力（下一层会详细讲），也是 DSA 架构相比 CUDA 逐 kernel 下发更接近"一次调用"的地方。

- **逐算子编译 + 多算子调用**：但在很多实际场景中，算子是被**单独编译**成 `.om` 的，host 侧需要按逻辑顺序**多次调用** `aclmdlExecute`，每次执行一个算子，中间还要管理显存和数据搬运。此时 host 的调用次数与算子数量成正比，和 CUDA 的逐 kernel 下发在"调用频繁度"上类似，只是粒度从"kernel"变成了"算子 .om"。

- **训练 / 多 step 场景**：训练循环每步迭代都要重新喂数据、取梯度，所以即便模型编译成单个 `.om`，也做不到"整个训练只调用一次"——而是"每个 step 调用一次 Execute"。
...
回顾整个第三章，我们可以把华为昇腾的 DSA 实践提炼为一条清晰的脉络。昇腾的达芬奇架构，是国产 DSA 路线中最具代表性的样本——它通过"按计算类型分离"的 Cube/Vector/Scalar 三单元设计，把 AI 负载中最密集、最规整的矩阵运算用专用硬件做到极致，同时将硬件无法承担的调度复杂度前移到了编译器（GE 图引擎）和开发者（Ascend C 编程模型）身上。这种"硬件专用化 + 软件复杂化"的权衡，直接塑造了昇腾软件栈的独特形态：上层（PyTorch 框架适配、集合通信 API、图优化）与 CUDA 生态高度趋同，让应用开发者几乎零成本迁移；底层（kernel 编程语言、编译模型）则与 CUDA 本质分化，要求算子开发者接受显式数据流的编程方式。

更关键的是，昇腾的案例揭示了一个可推广的规律：**DSA 架构的软件栈形态，归根结底由硬件架构决定**。达芬奇的静态调度特性，决定了昇腾必须采用"整图离线编译 + 图下沉执行"的模式，也决定了它无法像 GPGPU 那样通过 AST 替换直接承接 CUDA 生态。这一规律不仅适用于昇腾，也适用于所有走 DSA 路线的芯片——而接下来要讨论的 Google TPU，正是 AI芯片DSA领域当之无愧的鼻祖。TPU 比昇腾更早地实践了"专用架构 + 自建生态"的路线，但选择了与昇腾截然不同的策略——不是"有限兼容"，而是"完全自主"。通过对比昇腾与 TPU，我们将能更完整地看清 DSA 路线在"兼容 vs 自主"光谱上的两种极端选择。

---

## 四、案例二：Google TPU —— 完全自主生态的 DSA 路线

如果说华为昇腾走的是"在 DSA 硬件上有限兼容 CUDA 生态"的折中路线，Google TPU 则走向了**更加脱离于 CUDA 生态的方向**——从指令集、编译器到框架，构建了一套完全自主的全栈生态。需要澄清的是，这并非 Google "刻意选择了一条截然相反的路"，而是**历史时序决定的必然**：TPU 于 2015 年问世，彼时正是 CUDA 生态如日中天的时期，但 Google 从一开始就没有把"兼容 CUDA"作为目标——因为它走的是内部自用 + 云服务路线，不需要吸引已有 CUDA 开发者生态。相比之下，华为昇腾是 2018 年才入场的后来者，面对的是 CUDA 已经高度成熟的格局，因此在 DSA 硬件之上取了一个折中方案——有限地兼容 CUDA 生态（框架层 API 对齐），以尽可能降低开发者迁移的舒适度门槛。两者的差异，本质上是"自主生态先行者"与"兼容生态后来者"在战略定位上的不同。

在本节中，作者将带大家从**软件栈**的视角对比 TPU 的整体方案。需要说明的是，本文在前面已经对 DSA/GPGPU 软件栈的整体框架、以及华为 CANN 软件栈做了非常详细的拆解，因此这一节会尽量减少重复的逐层罗列，而是更多地提供一些**架构性的思考与总结**。强烈建议对 TPU 完全不了解的读者，先阅读作者的另外一篇笔记 [《GPU、TPU 与 LPU：AI 时代计算架构的分野》](https://lejun-hu.github.io/notes/gpu-tpu-lpu-architecture/) 学习 TPU 的基础概念（如数据流架构、脉动阵列 MXU、内存系统的设计哲学等），本文这里就不再赘述这些基础知识，而是直接聚焦于软件栈层面的分析。

### 4.1 TPU 的硬件与编译器栈

TPU 从 2015 年的第一代起就采用了**脉动阵列（Systolic Array）**作为核心计算单元——一种多个处理单元以规则网格排列、数据按固定节奏在单元间级联流动的专用结构。每一代 TPU 在脉动阵列尺寸、HBM 容量和互联拓扑上都有升级，但核心架构路径从未改变。最新的 Ironwood（TPU v7，2025 年发布）单芯片 FP8 算力达 4,614 TFLOPS，BF16 算力约 2,307 TFLOPS，配备 192GB HBM3e 显存、约 7.4 TB/s 内存带宽，是首款专为 AI **推理**设计的 TPU。作为参照，NVIDIA 同代竞品 Blackwell 架构的 B200 GPU BF16 算力约 2,250 TFLOPS——两者在 BF16 算力上处于同一量级，但定位不同：Ironwood 是推理专用（牺牲部分训练通用性换取极致推理吞吐），B200 则是训练与推理兼顾的通用加速器。
...
这里有几个名词需要澄清，它们构成了 TPU 软件栈的完整编译链路：

- **JAX 与 TensorFlow 的关系**：两者是 Google 生态中两个不同的高层框架。TensorFlow 是更早的深度学习框架，采用"先构图（graph）再执行"的模式；JAX 则是后来推出的、以函数式编程为核心的框架（`@jax.jit` 装饰器触发编译）。它们的共同点是**都使用 XLA 作为底层编译器**——用户在任一框架中定义计算，框架会把计算"翻译"成 XLA 能理解的中间表示，XLA 再统一编译到 TPU/GPU 上执行。因此 XLA 是这两个框架共享的"编译后端"，JAX 与 TensorFlow 在编译层是"殊途同归"的关系。

- **HLO（High-Level Optimizer IR，高层优化器中间表示）**：XLA 编译器使用的第一层中间表示。当 JAX/TensorFlow 把用户代码翻译给 XLA 时，首先得到的就是 HLO——它是一种与硬件无关的、描述"计算图"的 IR，粒度是算子级别（如 `dot`、`convolution`、`reduce`）。XLA 在 HLO 层做与硬件无关的通用优化，如算子融合、常量折叠、公共子表达式消除等。

- **LLO（Low-Level Optimizer IR，低层优化器中间表示）**：HLO 经过通用优化后，会被"降级"（lowering）为 LLO——一种更贴近具体硬件的 IR，此时已经绑定到目标设备（如 TPU）的指令粒度，描述了数据如何在脉动阵列、VMEM、HBM 之间流动。LLO 最终被编译为 TPU 原生的机器指令。

因此 TPU 的完整编译链路是：**JAX/TensorFlow 用户代码 → HLO（硬件无关的高层 IR）→ LLO（硬件相关的低层 IR）→ TPU 原生指令 → TPU Runtime 加载执行**。这条链路的本质，是把"高层框架的通用计算描述"逐步降级为"达芬奇/脉动阵列硬件能直接执行的确定性数据流"——每一层都比上一层更贴近硬件，同时信息也更具体。
...
这种体验对习惯于 PyTorch + CUDA 的开发者来说，迁移成本体现在**编程范式而非 API 对接**上。要理解这个差异，需要先解释"函数式编程"到底意味着什么。在 PyTorch 的命令式风格里，你写代码就像在写一段流程：`x = x + 1` 会立刻执行，变量可以被反复赋值修改，代码的执行顺序就是你的书写顺序——这很直观，但编译器难以提前"看清"整个计算的全貌。而 JAX 采用的是函数式风格：它要求你把计算组织成一个个**纯函数**，所谓"纯"指的是函数**没有任何副作用**——输入相同就一定输出相同，函数内部不会偷偷修改外部的变量或状态。

用一个例子就能直观感受两者的差异。同样是"把数组里的每个元素加 1 再取平方"，命令式风格是这样的：

```python
# 命令式风格（PyTorch）：变量被反复修改，代码顺序就是执行顺序
x = torch.tensor([1.0, 2.0, 3.0])
x = x + 1            # 立即执行，x 被覆盖
x = x * x            # 立即执行，x 再次被覆盖
print(x)             # tensor([4., 9., 16.])
```

而函数式风格要求你把计算封装成一个"输入→输出"的纯函数，中间没有对外部变量的修改：

```python
# 函数式风格（JAX）：计算封装为纯函数，输入相同输出必相同
def transform(x):
    return (x + 1) ** 2   # 只通过返回值传递结果，不修改任何外部状态

x = jnp.array([1.0, 2.0, 3.0])
y = transform(x)          # y 是结果，x 本身保持不变
print(y)                  # [4., 9., 16.]
```

对比两者的关键差异：命令式版本里 `x` 被改写了两次，同一个名字在不同时刻指向不同内容，编译器很难追踪"此刻的 x 到底是多少"；而函数式版本里 `transform` 是一个"给 `x` 就返回 `y`"的稳定映射，没有任何隐藏的中间状态。正是这种"无副作用"的确定性，让 JAX 可以放心地用 `@jax.jit` 装饰 `transform`，把它整体 trace 成一张静态计算图交给 XLA 做全局优化——因为无论编译器怎么重排、融合、并行化这个函数的内部运算，只要输入 `x` 不变，输出 `y` 就一定不变。这背后的逻辑是：只有函数"纯"了、行为可预测了，编译器才敢放心大胆地重排、融合、并行化你的计算。

理解了纯函数之后，我们再来看 JAX 生态中另外两个重要的概念。在实际的深度学习任务里，我们几乎从来不会只处理单个样本或单个设备——训练时数据是一批一批喂进来的，大规模模型还要跨很多张卡并行。JAX 应对这两类"规模化"需求的方式，就是两个建立在纯函数之上的转换操作：

`vmap`（vectorization map，自动向量化）解决的是"单个样本 → 一批样本"的问题。它让你只写一个"处理单个样本"的纯函数，JAX 会自动把这个函数扩展到"并行处理一整批样本"，你不需要手写 batch 循环。`pmap`（parallel map，并行映射）则更进一步，解决"单设备 → 多设备"的问题——它把数据切分到多个设备（如多张 TPU）上，让每个设备运行同一份函数、各自处理不同的数据分片。这正是 **SPMD**（Single Program Multiple Data，单程序多数据）的体现：所有设备执行的是同一份代码，只是各自处理不同的数据切片。

`vmap` 和 `pmap` 之所以能这么"自动"，根源仍然在于纯函数：正因为函数没有副作用、输入输出关系确定，编译器才能放心地把"单设备单样本"的逻辑，机械地推广到"多设备多样本"，而不必担心数据之间的相互干扰。

由此可以看出，从 PyTorch 迁移到 JAX 的核心挑战，是**思维方式的转变**——从"写一段顺序执行的流程"变成"写一个可被编译器自由转换的纯函数"，记 `jit`/`vmap`/`pmap` 这几个 API 本身反而是次要的。不过对于已经在 JAX 生态中的用户（如 Google DeepMind 和许多学术研究团队），设备差异是完全透明的——同一份纯函数代码不经修改即可在 TPU 或 GPU 上运行，XLA 负责后端适配。

### 4.3 TPU 软件栈的全貌：从算子到框架的另一种组织方式

在了解了 XLA 的编译链路和 JAX 的编程范式之后，我们再从几个更宏观的问题出发，把 TPU 软件栈的完整形态串起来。

**TPU 有没有"算子"的概念？** 答案是有的，但它的"算子"与我们前面在 CUDA 生态里反复提到的算子（如 cuBLAS 里的 GEMM、cuDNN 里的卷积）含义不同。在 TPU 的软件栈里，算子的概念出现在 **XLA 的 HLO 层**——HLO 本身就是一张由算子组成的图，每个节点（如 `dot` 表示矩阵乘、`convolution` 表示卷积、`reduce` 表示规约）就是一个算子。但关键区别在于：CUDA 生态里的算子是"**预先写好的、针对特定硬件手工调优的 kernel**"（cuBLAS/cuDNN 里的闭源实现），而 TPU 的 HLO 算子是"**编译器层面的抽象描述**"——它描述的是"要做什么数学运算"，而不是"这段运算的机器码长什么样"。真正把 HLO 算子翻译成 TPU 可执行指令的，是 XLA 后端的代码生成器。换句话说，CUDA 的算子库是把"调优好的实现"打包成库供你调用，TPU 的算子是让编译器"当场生成最优实现"。

**TPU 能不能支持 PyTorch？** 能。这一点常常被误解——很多人以为 TPU 只能跑 TensorFlow/JAX，实际上 Google 通过 **PyTorch/XLA** 项目提供了完整的 PyTorch 支持：开发者用标准的 PyTorch API 写代码，PyTorch/XLA 会把这部分代码也翻译成 XLA 的 HLO，再走和 JAX/TensorFlow 完全相同的编译链路，最终在 TPU 上执行。所以 TPU 上其实可以运行 PyTorch、TensorFlow、JAX 三种主流框架，它们共享同一个 XLA 后端。

这也顺带回答了一个更根本的问题——**TensorFlow 与 PyTorch 在底层思想上有很大不同吗？** 从框架的"上层编程风格"看，两者确实不同，用一个最简单的例子就能直观感受：同样是"定义两个张量相加"，

```python
# PyTorch（动态图 / eager）：代码立即执行，每行都马上算出结果
import torch
x = torch.tensor([1.0, 2.0])
y = torch.tensor([3.0, 4.0])
z = x + y            # 这行代码执行到此处，z 立刻被计算出来
print(z)             # 马上能看到 tensor([4., 6.])
```

```python
# TensorFlow 1.x（静态图）：先"描述"计算图，之后才"真正执行"
import tensorflow as tf
x = tf.placeholder(tf.float32, [2])   # 占位符：先声明"这里会有一个输入"
y = tf.placeholder(tf.float32, [2])
z = x + y                             # 这一步只是"构图"，并没有真的做加法
with tf.Session() as sess:            # 只有在 Session 里 run 时才真正执行
    print(sess.run(z, feed_dict={x: [1.0, 2.0], y: [3.0, 4.0]}))
```

在 PyTorch 里，`z = x + y` 这行代码执行时，加法就已经真实发生了，你可以立刻打印结果——这就是"即时执行"（eager）。而在 TensorFlow 1.x 里，`z = x + y` 只是往计算图里**添加了一个加法节点**，并没有任何数值被计算；真正的计算要等到 `sess.run(...)` 时才会发生。这两种风格各有优劣：eager 模式写起来直观、调试方便，但编译器难以提前看到整个计算的全貌；静态图模式写起来更繁琐，但整张图在运行前就完整可见，编译器可以做更充分的优化。

不过从 TPU 软件栈的视角看，这个"上层风格"的差异在 XLA 这一层被**统一抹平了**——无论你用的是 PyTorch 的 eager 还是 TensorFlow 的静态图，最终都会被降级为同一套 HLO，走同一条编译链路。也就是说，框架之间的差异是"前端"的差异，而 XLA 提供的是一套"后端统一"的抽象，让不同框架殊途同归。

这一特性恰恰是 TPU 生态最值得我们深思的地方。对比昇腾的"有限兼容"策略——它是在 PyTorch 这一层做适配（`torch_npu` 劫持 dispatcher），让上层框架保持不变、底层替换成自研实现；而 Google 走的是另一条路——**提供一个统一的编译器抽象（XLA），让所有框架都适配到这个抽象上**。两者的共同点是"把兼容成本集中在一处"，但落点不同：昇腾落在"框架适配层"，Google 落在"编译器抽象层"。这解释了为什么 TPU 不需要"兼容 CUDA"——它兼容的是"框架"（通过 XLA 承接 PyTorch/TF/JAX），而不是"CUDA 这套底层 API"。

理解了这个全貌，TPU 路线的本质就清晰了：**它用"编译器抽象（XLA）"取代了"硬件 API（CUDA）"作为生态的锚点。** 框架只需适配 XLA，就能获得 TPU 加速能力，而 XLA 内部负责所有与硬件相关的编译和优化。这种"以编译器为中心的生态"与 CUDA "以 API 为中心的生态"，是两种截然不同的生态组织哲学——前者把复杂度藏进编译器，后者把复杂度暴露在 API 上。

顺着这个思路，一个很自然会浮现的疑问是：华为昇腾和 Google TPU 同属 DSA 架构，为什么昇腾需要开发者专门手写算子、再用 ATC 编译成 `.om`，而 TPU 的开发者却似乎"不需要手写算子"？要理解这一点，需要区分"**编译这件事由谁触发、对开发者是否可见**"。

两者都是 DSA，都"必须先编译再执行"（因为硬件都是静态调度，无法像 CUDA 那样即时下发 kernel），这是共同的底线。真正的分歧在于：昇腾保留了类似 CUDA 的"开发者写算子 → 编译 → 加载执行"工作流，编译动作是**显式、可控、开发者主导**的（ATC 工具 + `.om` 文件），让有 kernel 开发需求的用户能介入底层；而 TPU 把编译**隐式、自动、编译器主导**地完成了——`@jax.jit` 首次执行时 XLA 自动编译，产物被运行时缓存并直接加载，开发者感知不到那个"编译好的二进制"。换句话说，`.om` 不是 DSA 的必然要求，而是昇腾"有限兼容 CUDA"策略下的选择；TPU 用"自动 JIT 编译 + 透明缓存"达到了同样的效果，只是把编译藏进了编译器。

但这绝不意味着 TPU "不需要考虑算子性能"。恰恰相反，算子优化在 TPU 里依然存在，只是**责任从开发者转移到了编译器团队**：XLA 团队仍然要花大量精力让 `dot`、`convolution` 这些 HLO 算子在脉动阵列上跑得快，只是这些优化对所有用户共享、统一生效，普通开发者无需感知。当需要一个新的、XLA 未优化好的算子时，TPU 生态里你的做法不是"写一个 kernel"，而是"给 XLA 写一个优化规则（pass）或降级规则"——这是更底层、更小众的工作。由此带来的 trade-off 是：昇腾把"算子性能优化"的责任部分留给了开发者（手写 Ascend C），换来的是深度定制能力和灵活性；TPU 把责任几乎全部收归编译器，换来的是普通开发者的省心，代价则是灵活性下降、深度定制难——如果你需要的算子 XLA 没优化好，往往只能等 Google 优化编译器，自己很难插手。这正是"以 API 为中心（开发者可介入）"与"以编译器为中心（编译器全权负责）"两种哲学在算子层面的具体体现。

---

## 五、案例三：摩尔线程 MUSA —— GPGPU 路线的"极致 API 兼容"

在前面两章中，我们分别剖析了华为昇腾（DSA 路线、有限兼容）和 Google TPU（DSA 路线、完全自主）两种策略。而摩尔线程 MUSA 则代表了第三条路线——**GPGPU 架构下的极致 API 兼容**。如果说昇腾和 TPU 都在某种程度上"放弃"或"重构"了 CUDA 的底层编程模型，那么摩尔线程的策略则截然相反：它的核心目标，就是让已经习惯 CUDA 的开发者，能够以**最小的摩擦**迁移到自己的硬件上。

这就引出了理解摩尔线程的一个关键问题：**API 级兼容 CUDA，是否就是摩尔线程最核心的技术路线？** 答案是肯定的。整个 MUSA 生态的技术投入，几乎都围绕着一个目标展开——**尽可能追求"CUDA 用户无缝切换"**。摩尔线程清楚地认识到，作为国产 GPU 领域的后来者，它无法在短期内建立像昇腾那样庞大的自研框架生态，也无意像 TPU 那样另起炉灶；它唯一可行的竞争策略，就是**承接 NVIDIA 已经培育了二十年的 CUDA 开发者存量**——让那些写着 `cudaMalloc`、`<<<>>>`、`cublasGemmEx` 的代码，几乎不改就能跑在自己的卡上。理解了这一点，MUSA 的各层设计就都有了统一的逻辑主线。

### 5.1 MUSA 的分层兼容策略

MUSA 的兼容是在每一层都做对应映射，而非只在某一层"一刀切"。而这种"逐层映射"的最终目的，都是为了把"切换成本"压到最低：

**Runtime 层**：MUSA Runtime 提供 `musaMalloc`、`musaMemcpy`、`musaLaunchKernel` 等 API，语义与 CUDA Runtime 完全对应。MUSA SDK 5.1.0 对标 CUDA 12.8，兼容接口数达 761 个（涵盖驱动、运行时和核心数学库）。这一层的目标是让"函数调用层面"零障碍——开发者几乎只需要做一次函数名替换。

**编译器层**：MUSA 编译器支持 CUDA C++ 扩展语法（`__global__`、`<<<>>>` 等），CUDA 源码经 **MUSIFY**（代码自动迁移工具）转换后即可编译为 MUSA 指令。MUSIFY 的转换实质上是一次**AST 级别的函数名替换**（`cudaMalloc` → `musaMalloc`），因为两者在编程模型上同构（都是 SIMT 线程模型），转换不涉及语义重构。这一层是摩尔线程"无缝切换"最核心的支撑——正因为 GPGPU 与 CUDA 的编程模型同构，才可能做到"机械替换函数名即可迁移"，而不像昇腾那样需要开发者学习全新的 Ascend C。
...
**加速库层**：摩尔线程提供 muDNN 对标 cuDNN（宣称 GEMM/FlashAttention 效率超过 98%）、muBLAS 对标 cuBLAS、MCCL 对标 NCCL（支持 AllReduce/AllGather 等全部原语，双机 RDMA 带宽利用率达 97%）。这一层的实际工作量极大——需要独立实现每个库中的全部核心 API 和底层 kernel，以满足数值精度和性能上的对标要求。摩尔线程采用 API 级干净室重实现的策略，即提供同名的自有 `.so` 文件并保证符号表和语义完全对应，但内部实现全自研。这一层的意义在于：让那些依赖 cuBLAS/cuDNN 的**上层框架和第三方库**（它们并不直接写 kernel，而是调用这些闭源库的 API）也能无缝迁移。

**框架层**：MUSA 已正式成为 vLLM 官方后端，并合入 SGLang 官方主线。对 PyTorch 用户而言，迁移成本仅为将 `device="cuda"` 改为 `device="musa"`（或通过适配层自动劫持映射）。这是"无缝切换"的最终落地——把前面各层的兼容工作，收敛成用户视角下"改一个字符串"的体验。

### 5.2 MUSA 路线的优势与边界

MUSA 路线最大的优势在于开发者体验——从 CUDA 生态迁移的摩擦成本在国产方案中最低。这正是"API 级兼容"策略的直接回报：它不需要开发者学习新的编程模型（对比昇腾的 Ascend C）、不需要换一套框架思维（对比 TPU 的 JAX/XLA），甚至大部分代码只需一次函数名替换。

但"无缝切换"并非没有边界，它存在几个明确的极限：

- **需要重新编译**：MUSA 兼容的是"源码层面"的 CUDA，而非"二进制层面"。已经编译好的 CUDA 可执行文件/`.so`（内含 PTX/SASS）无法直接运行，必须经过 MUSIFY 转换 + 重新编译。这意味着摩尔线程无法承接"没有源码的闭源二进制"。

- **内联 PTX 汇编无法转换**：对于使用了 `asm("...")` 内联 PTX 汇编的 CUDA 代码，MUSIFY 无法自动转换——因为这些汇编指令直接绑定了 NVIDIA 硬件的特定指令，MUSA 硬件上不存在一一对应的指令。

- **部分硬件特性的差异**：CUDA 特有的硬件特性（如特定代数 Tensor Core 的 `wmma` 指令）对应的 MUSA 硬件实现可能不完全一致，需要手动调整。

在行业采纳方面，摩尔线程的 MUSA 已被德锐特（德睿智药子公司）用于冷冻电镜数据处理软件 RELION5 的迁移，双机 RDMA 通信带宽利用率达到 97%。在 PyTorch 生态中，MUSA 已完整支持全部 3,194 个算子。

### 5.3 小结：GPGPU 路线"无缝切换"的本质

回顾摩尔线程的整个策略，可以提炼出一个清晰的结论：**API 级兼容 CUDA 确实是摩尔线程最核心、也是唯一可行的技术路线。** 这并非摩尔线程"偷懒"或"缺乏创新"，而是由它的两个现实约束决定的——其一是**硬件架构选择了 GPGPU**（与 CUDA 编程模型同构，天然适合做 API 映射），其二是**市场定位决定了它必须承接 CUDA 的存量生态**（作为后来者，自建生态的成本和风险都不可承受）。

这条路线与昇腾、TPU 形成了鲜明的三角对照：昇腾在 DSA 硬件上"有限兼容"（框架层对齐、kernel 层分化），TPU 在 DSA 硬件上"完全自主"（XLA 编译器统一后端），而摩尔线程在 GPGPU 硬件上"极致兼容"（API 层全映射）。三者的共同启示是：**硬件架构决定了兼容策略的"可能性"，市场定位决定了兼容策略的"必要性"，而最终的软件栈形态，是这两者共同作用的结果。** 摩尔线程选择 GPGPU，就注定它最大的筹码是"让 CUDA 用户无缝迁移"；而它放弃 DSA，也就放弃了 DSA 架构在能效比上的潜在优势——这是一场用"生态兼容性"换取"硬件通用性"的明确取舍。
...
**1. CUDA 不是一层 API，而是一个多层次软件帝国。** 从 PyTorch 框架到 GPU 硬件，数据流经过了框架调度层、闭源算子库、Runtime/Driver API、PTX 中间表示、SASS 机器码、FATBIN 多架构打包、以及 Runtime 加载与执行。每一层都有独立的技术壁垒——闭源的 cuBLAS 和 cuDNN 的自动调优引擎、libcuda.so 中的数百个 Dark API、NVLink 与 InfiniBand 的私有互联协议、以及数十年来积累的特定架构 SASS 编码优化。此外，多卡场景下的集合通信（NCCL）和通算融合，是这条链路上另一条容易被忽视但同样关键的分支——它把"单卡计算"扩展为"跨卡协同"，其编译方式（CPU 端 GCC + GPU 端 nvcc 混合编译进同一个 `.so`）和与计算库的协作机制，同样是理解 CUDA 生态全貌不可或缺的一环。理解这种多层次结构，是评估任何兼容方案可行性的前提。
...
**3. 硬件架构从根本上塑造了软件栈的形态。** 这是贯穿三个案例的最深层规律。达芬奇的"按类型分离"静态调度，决定了昇腾必须采用"整图离线编译 + 图下沉执行"、必须让开发者用 Ascend C 显式管理数据流；TPU 的脉动阵列数据流，决定了它必须依赖 XLA 做全程序静态编译；而摩尔线程的 GPGPU 架构与 CUDA 编程模型同构，才使得"API 级函数名替换"成为可能。反过来看，软件栈的每一处差异，几乎都能追溯到硬件有没有动态调度器、存储是否功能绑定、矩阵计算是不是独立单元这些底层特征。理解了这一点，就能理解为什么"兼容 CUDA 程度"这个宣传话术并不足以判断一个芯片的软件栈形态——真正起决定作用的是底层硬件架构。

**4. 三个案例展示了三种不同的兼容策略，但它们共享同一种生态组织逻辑的抉择。** 华为昇腾 CANN 在 DSA 硬件上走"有限兼容"——框架层和 Runtime 层提供类 CUDA 体验，kernel 编程层做完全不同的 Ascend C，把兼容成本集中在"框架适配层"。Google TPU 完全放弃 CUDA 兼容，转而通过 JAX + XLA 构建独立生态，用"编译器抽象（XLA）"取代"硬件 API（CUDA）"作为生态锚点，把复杂度藏进编译器。摩尔线程 MUSA 在 GPGPU 硬件上走"极致 API 兼容"——通过 MUSIFY 工具、自研编译器、自有加速库和完整 PyTorch 适配，把迁移成本压缩到改 device 名字符串的程度。三条路线的共同启示是：**硬件架构决定了兼容策略的"可能性"，市场定位决定了兼容策略的"必要性"，而最终软件栈形态是这两者共同作用的结果**——不存在普适的"最优解"，只有与自身禀赋匹配的"最适合解"。

**5. 算子与 kernel 的边界，是理解三个生态差异的关键切口。** CUDA 的算子是"预先编译好的、打包在闭源库里的 kernel"（运行时动态链接选中）；昇腾的算子是"离线编译好的 `.om` 文件"（运行时加载执行）；TPU 的算子是"编译器层面的 HLO 抽象节点"（XLA 编译期现场生成机器码）。这个看似细节的差异，实际上折射出三种生态对"算子的性能优化责任由谁承担"这一问题的不同回答——CUDA 交给闭源库团队，昇腾部分交给开发者（手写 Ascend C），TPU 几乎全部交给编译器（XLA）。它决定了开发者在每个生态里"能介入到什么深度、又能省心到什么程度"。

**6. 产业趋势：图编译模式的普及对 DSA 有利。** PyTorch 2.0 的 `torch.compile` 默认启用图模式编译、vLLM 等推理框架在向编译派发模式演进，整个 AI 软件栈正在从逐 kernel 的 eager 执行转向全局图优化。图编译模式下，上层框架主动交出完整的子图结构，恰好是 DSA 编译器（如 CANN GE、Google XLA）的标准输入形态——无需在运行时动态转换线程语义。这一趋势客观上缩小了 DSA 架构在"迁移痛感"上与 GPGPU 路线的差距——但需要指出的是，对于存量 CUDA 源码（特别是涉及自定义 kernel 和 warp 级编程的代码），GPGPU 路线的直接兼容优势仍然不可替代。

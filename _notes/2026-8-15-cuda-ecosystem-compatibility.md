...
在了解了概念之后，我们进入达芬奇架构的硬件细节。首先要说明的是，本文在第二章对比 GPGPU 与 DSA 时，是从"跨厂商"的宏观视角讨论的；而这一节要澄清一个容易混淆的点——在昇腾内部，也有一套自己的"耦合架构 vs 分离架构"术语，它指的是**达芬奇架构内部 AI Core 的两种组织方式**，而不是 NVIDIA 与昇腾之间的对比。

具体来说，昇腾的 AI Core 硬件架构根据 **Cube（矩阵）计算单元和 Vector（向量）计算单元是否同核部署**，分为两种模式：

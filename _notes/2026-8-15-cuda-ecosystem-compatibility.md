...
在本节中，作者将带大家从**软件栈**的视角对比 TPU 的整体方案。需要说明的是，本文在前面已经对 DSA/GPGPU 软件栈的整体框架、以及华为 CANN 软件栈做了非常详细的拆解，因此这一节会尽量减少重复的逐层罗列，而是更多地提供一些**架构性的思考与总结**。如果你想了解 TPU 更详细的硬件架构与设计思路（例如它的数据流架构、脉动阵列 MXU 以及内存系统的设计哲学），可以阅读作者的另外一篇笔记 [《GPU、TPU 与 LPU：AI 时代计算架构的分野》](https://lejun-hu.github.io/notes/gpu-tpu-lpu-architecture/)。

...
更关键的是，昇腾的案例揭示了一个可推广的规律：**DSA 架构的软件栈形态，归根结底由硬件架构决定**。达芬奇的静态调度特性，决定了昇腾必须采用"整图离线编译 + 图下沉执行"的模式，也决定了它无法像 GPGPU 那样通过 AST 替换直接承接 CUDA 生态。这一规律不仅适用于昇腾，也适用于所有走 DSA 路线的芯片——而接下来要讨论的 Google TPU，正是 DSA 领域当之无愧的鼻祖。TPU 比昇腾更早地实践了"专用架构 + 自建生态"的路线，但选择了与昇腾截然不同的策略——不是"有限兼容"，而是"完全自主"。通过对比昇腾与 TPU，我们能更完整地看清 DSA 路线在"兼容 vs 自主"光谱上的两种极端选择。

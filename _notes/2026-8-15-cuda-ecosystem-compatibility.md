...
理解了纯函数，`vmap` 和 `pmap` 就顺理成章了。它们本质上都是"如何把同一个纯函数应用到更多数据/设备上"的转换操作：`vmap`（vectorization map，自动向量化）让你写一个"处理单个样本"的函数，JAX 自动把它扩展到"并行处理一批样本"，无需手写 batch 循环；`pmap`（parallel map，并行映射）则更进一步，把数据切分到多个设备（如多张 TPU）上，让每个设备跑同一份函数、处理不同的数据分片——这正是 **SPMD**（Single Program Multiple Data，单程序多数据）的体现：所有设备执行同一份代码，只是各自处理不同的数据切片。而这一切之所以能自动化，根源还是因为函数是纯的、无副作用的——编译器可以放心地把"单设备单样本"的逻辑，机械地推广到"多设备多样本"。

所以从 PyTorch 迁移到 JAX 的核心障碍，不在于记不住 `jit`/`vmap`/`pmap` 这几个 API 名字，而在于**思维方式的转变**：从"写一段顺序执行的流程"变成"写一个可被编译器自由转换的纯函数"。不过对于已经在 JAX 生态中的用户（如 Google DeepMind 和许多学术研究团队），设备差异是完全透明的——同一份纯函数代码不经修改即可在 TPU 或 GPU 上运行，XLA 负责后端适配。

...
**TPU 能不能支持 PyTorch？** 能。这一点常常被误解——很多人以为 TPU 只能跑 TensorFlow/JAX，实际上 Google 通过 **PyTorch/XLA** 项目提供了完整的 PyTorch 支持：开发者用标准的 PyTorch API 写代码，PyTorch/XLA 会把这部分代码也翻译成 XLA 的 HLO，再走和 JAX/TensorFlow 完全相同的编译链路，最终在 TPU 上执行。所以 TPU 上其实可以运行 PyTorch、TensorFlow、JAX 三种主流框架，它们共享同一个 XLA 后端。这也顺带回答了一个更根本的问题——**TensorFlow 与 PyTorch 在底层思想上有很大不同吗？** 从框架的"上层编程风格"看，两者确实不同：TensorFlow 早期是"先构图、后执行"的静态图风格，PyTorch 是"即时执行"的动态图（eager）风格。但从 TPU 软件栈的视角看，这个差异在 XLA 这一层被**统一抹平了**——无论上层是哪种风格，最终都会被降级为同一套 HLO，走同一条编译链路。也就是说，框架之间的差异是"前端"的差异，而 XLA 提供的是一套"后端统一"的抽象，让不同框架殊途同归。

...
- GPU 之间可能通过 **NVLink**（同机高速直连）、**PCIe**（同机较慢直连）、**RDMA 网卡（InfiniBand / RoCE）**（跨机）等多种物理链路相连。此外，其他公司自研的底层互联方案——例如 AMD 的 Infinity Fabric、国产网卡/IP 厂商自研的链路——也会在这一层接入，成为 NCCL 需要适配的"传输层"之一；
